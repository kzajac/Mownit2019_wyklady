...
Progresywna, bo pomiędzy $i$ oraz $i+1$ (do przodu):
  \begin{gather*}
    \Delta^{(0)}_{y_i} := y_i\\
    \Delta^{(k)}_{y_i} := \Delta^{(k - 1)}_{y_{i+1}} - \Delta^{(k - 1)}_{y_i}, k \ge 1
  \end{gather*}
...
\begin{itemize}
\item ilorazy róznicowe (przypomnienie):\\
 $x_{i}$ : $f[x_{i}]=f(x_{i})$ oraz
$
f[x_{i},\ x_{i+1}]=\frac{f[x_{i+1}]-f[x_{i}]}{x_{i+1}-x_{i}}
$
$$
...
$$f[x_{0},\displaystyle \ x_{1},\cdots,\ x_{k}]=\frac{1}{k!h^{k}}\triangle^{k}_{f(x_{0})}$$ \\

\item Podstawiając do (**) mamy:
$$P_{n}(x)=P_n(x_0+s\cdot h)=\displaystyle
\sum_{k=0}^{n}(_{k}^{s})\triangle^{k}_{f(x_{0})}$$
